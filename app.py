...
with tab1:
    ...
    with col_cards:
        ...
        cards = [
            ("👥 Total Employees", f"{total:,}",     "#1F6FEB"),
            ("✅ Stayed",          f"{stayed_count:,}", "#1a4d2e"),
            ("🚪 Left",            f"{left_count:,}",  "#4d1a1a"),
            ("📉 Attrition Rate",  f"{att_rate:.1%}",  "#4d3a1a"),
        ]
        for label, value, bg in cards:
            ...
            <div style="background-color:{bg}; border-radius:10px; padding:16px 20px;
                ...
                <div style="color:#A0ADB8; font-size:13px; margin-bottom:4px;">{label}</div>
                <div style="color:#E6EDF3; font-size:26px; font-weight:600;">{value}</div>
            </div>
            """, unsafe_allow_html=True)
...
with tab5:
    ...
    **Profile:** Entry-level · No remote work · No leadership opportunities · No innovation opportunities · Overtime = Yes
    ...
    st.caption("Top 3 drivers ranked by spread between best and worst group attrition rate.")

    drivers = {
        "Job Level":          43.0,
        "Remote Work":        28.1,
        "Work-Life Balance":  24.5,
        "Job Satisfaction":    8.0,
        "Overtime":            6.0,
        "Innovation Opps":     3.0,
        "Leadership Opps":     2.8,
    }
    drivers_df = pd.DataFrame(list(drivers.items()), columns=["Factor", "Spread (pp)"])
    drivers_df = drivers_df.sort_values("Spread (pp)", ascending=True)
