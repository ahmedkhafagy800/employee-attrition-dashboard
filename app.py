...
# ── Theme Toggle (manual — st.get_option is unreliable) ──────────
if "dark_mode" not in st.session_state:
    st.session_state.dark_mode = True   # default: Dark

# We render the toggle AFTER sidebar title, but we need the value NOW
...
# ── Global CSS (injected after theme is decided) ──────────────────
st.markdown(f"""
<style>
    /* Page background */
    ...
    [data-testid="stSidebar"] .stToggle p {{ color: white !important; }}

    /* Headings */
    h1, h2, h3, h4 {{ color: {BRAND['primary']} !important; }}

    /* Body text */
    p, span, div, label {{ color: {BRAND['font_color']}; }}

    /* Tabs */
    .stTabs [data-baseweb="tab-list"] {{
        background-color: {BRAND['tab_list_bg']};
        border-radius: 8px;
        padding: 4px;
    }}
    ...
    /* Caption text */
    .stCaption, small {{ color: {BRAND['font_color']} !important; opacity: 0.7; }}

    /* Info boxes */
    [data-testid="stAlert"] {{
        background-color: {'#1C2A3A' if is_dark else '#EFF6FF'} !important;
...
<div style="display:flex; align-items:center; padding:16px 0 24px 0;
    ...
    {logo_html}
    <div>
        <h1 style="margin:0; color:{BRAND['primary']}; font-size:2rem;">
