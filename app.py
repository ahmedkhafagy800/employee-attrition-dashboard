...
BRAND = {
    'primary':     '#58A6FF',
    'secondary':   '#1F6FEB',
    'light':       '#161B22'   if is_dark else '#E8ECF0',
    'bg':          '#0D1117'   if is_dark else '#FFFFFF',
    'stayed':      '#3FB950'   if is_dark else '#1a7a32',
    'left':        '#F85149'   if is_dark else '#d63031',
    'chart_title': '#58A6FF',
    'palette':     ['#58A6FF', '#3FB950', '#F85149', '#D2A8FF', '#FFA657'],
    'font_color':  '#E6EDF3'   if is_dark else '#0D1117',
    'grid_color':  '#2A3A4A'   if is_dark else '#E0E0E0',
    'line_color':  '#444444'   if is_dark else '#CCCCCC',
    'paper_bg':    '#0D1117'   if is_dark else '#FFFFFF',
    'plot_bg':     '#161B22'   if is_dark else '#F8F9FA',
...
<style>
    ...
    [data-testid="metric-container"] {{
        background-color: {'#161B22' if is_dark else '#F0F2F6'};
...
with tab1:
    ...
    with col_cards:
        ...
        cards = [
            ("👥 Total Employees", f"{total:,}",      "#1F6FEB" if is_dark else "#dbeafe"),
            ("✅ Stayed",          f"{stayed_count:,}", "#1a4d2e" if is_dark else "#dcfce7"),
            ("🚪 Left",            f"{left_count:,}",  "#4d1a1a" if is_dark else "#fee2e2"),
            ("📉 Attrition Rate",  f"{att_rate:.1%}",  "#4d3a1a" if is_dark else "#fef3c7"),
        ]
